...
export default function AppWizard({ isActive }) {
  ...
  const saveConfiguration = async () => {
    ...
    const currentConfig = { appName, port, features, onboardingSteps, selectedAuthEmbed, selectedPaymentEmbeds, routePermissions, customPages, branding };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const allSelectedTokens = [
        selectedAuthEmbed,
        ...selectedPaymentEmbeds,
        ...customPages.flatMap(p => p.embeds || [])
      ].filter(Boolean);
    ...
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Select an existing app to load its configuration, or fill out the form to generate a new one.
        </p>
        <div className="mt-4">
